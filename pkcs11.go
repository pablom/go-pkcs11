...
struct ctx *New(const char *module)
{
	...
	c->handle = dlopen(module,  RTLD_LAZY | RTLD_LOCAL);
	if (c->handle == NULL) {
		free(c);
		return NULL;
	}
...
CK_RV GetMechanismList(struct ctx* c, CK_ULONG slotID, CK_ULONG_PTR * mech, CK_ULONG_PTR mechlen)
{
	CK_RV e = c->sym->C_GetMechanismList((CK_SLOT_ID) slotID, NULL, mechlen);
	...
	*mech = calloc(*mechlen, sizeof(CK_MECHANISM_TYPE));
	e = c->sym->C_GetMechanismList((CK_SLOT_ID) slotID,(CK_MECHANISM_TYPE_PTR) * mech, mechlen);
	return e;
}

CK_RV GetMechanismInfo(struct ctx* c, CK_ULONG slotID, CK_MECHANISM_TYPE mech, CK_MECHANISM_INFO_PTR info)
{
...
CK_RV SetPIN(struct ctx* c, CK_SESSION_HANDLE sh, char* oldpin, CK_ULONG oldpinlen, char* newpin, CK_ULONG newpinlen)
{
	CK_RV e = c->sym->C_SetPIN(sh, (CK_UTF8CHAR_PTR) oldpin, oldpinlen,(CK_UTF8CHAR_PTR) newpin, newpinlen);
	return e;
}
...
CK_RV GetSessionInfo(struct ctx* c, CK_SESSION_HANDLE session,CK_SESSION_INFO_PTR info)
{
...
{
	return c->sym->C_SetOperationState(session, state, statelen, encryptkey, authkey);
}
...
CK_RV CreateObject(struct ctx* c, CK_SESSION_HANDLE session, ckAttrPtr temp, CK_ULONG tempCount, CK_OBJECT_HANDLE_PTR obj)
{
...
CK_RV CopyObject(struct ctx* c, CK_SESSION_HANDLE session, CK_OBJECT_HANDLE o, ckAttrPtr temp, CK_ULONG tempCount, CK_OBJECT_HANDLE_PTR obj)
{
	ATTR_TO_C(tempc, temp, tempCount, NULL);
...
CK_RV GetAttributeValue(struct ctx* c, CK_SESSION_HANDLE session, CK_OBJECT_HANDLE object, ckAttrPtr temp, CK_ULONG templen)
{
...
CK_RV FindObjectsInit(struct ctx* c, CK_SESSION_HANDLE session, ckAttrPtr temp, CK_ULONG tempCount)
{
	ATTR_TO_C(tempc, temp, tempCount, NULL);
...
{
	CK_RV rv = c->sym->C_EncryptUpdate(session, plain, plainlen, NULL, cipherlen);
	if (rv != CKR_OK) {
		return rv;
	}
	*cipher = calloc(*cipherlen, sizeof(CK_BYTE));
	...
	rv = c->sym->C_EncryptUpdate(session, plain, plainlen, *cipher, cipherlen);
	return rv;
}
...
CK_RV DigestInit(struct ctx* c, CK_SESSION_HANDLE session,ckMechPtr mechanism)
{
	MECH_TO_C(m, mechanism);
...
CK_RV SignInit(struct ctx* c, CK_SESSION_HANDLE session, ckMechPtr mechanism, CK_OBJECT_HANDLE key)
{
	MECH_TO_C(m, mechanism);
...
CK_RV SignUpdate(struct ctx* c, CK_SESSION_HANDLE session, CK_BYTE_PTR message, CK_ULONG mlen)
{
	CK_RV rv = c->sym->C_SignUpdate(session, message, mlen);
	return rv;
}

CK_RV SignFinal(struct ctx* c, CK_SESSION_HANDLE session, CK_BYTE_PTR * sig, CK_ULONG_PTR siglen)
{
...
CK_RV SignRecoverInit(struct ctx* c, CK_SESSION_HANDLE session, ckMechPtr mech, CK_OBJECT_HANDLE key)
{
	MECH_TO_C(m, mech);
...
CK_RV VerifyInit(struct ctx* c, CK_SESSION_HANDLE session, ckMechPtr mech, CK_OBJECT_HANDLE key)
{
	MECH_TO_C(m, mech);
...
CK_RV VerifyUpdate(struct ctx* c, CK_SESSION_HANDLE session, CK_BYTE_PTR part, CK_ULONG partlen)
{
	CK_RV rv = c->sym->C_VerifyUpdate(session, part, partlen);
	return rv;
}

CK_RV VerifyFinal(struct ctx* c, CK_SESSION_HANDLE session, CK_BYTE_PTR sig, CK_ULONG siglen)
{
	CK_RV rv = c->sym->C_VerifyFinal(session, sig, siglen);
	return rv;
}

CK_RV VerifyRecoverInit(struct ctx* c, CK_SESSION_HANDLE session, ckMechPtr mech, CK_OBJECT_HANDLE key)
{
	MECH_TO_C(m, mech);
...
CK_RV SignEncryptUpdate(struct ctx* c, CK_SESSION_HANDLE session, CK_BYTE_PTR part, CK_ULONG partlen, CK_BYTE_PTR * enc, CK_ULONG_PTR enclen)
{
...
CK_RV WrapKey(struct ctx* c, CK_SESSION_HANDLE session,
	      ...
	      CK_OBJECT_HANDLE key, CK_BYTE_PTR * wrapped, CK_ULONG_PTR wrappedlen)
{
	MECH_TO_C(m, mechanism);
